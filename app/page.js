...
const Resume = () => {
  ...
  // Patents Data (2025 年專利已統一改為 "2025 (granted)")
  const patents = [
    {
  ...
  // Icon 統一採用高階科技藍 (sky-600)
  const certificates = [
    { title: "Certified Public Accountant (CPA)", icon: <Award className="w-4 h-4 text-sky-600" /> },
    { title: "Project Management Professional (PMP)", icon: <CheckCircle2 className="w-4 h-4 text-sky-600" /> },
    { title: "ISO/IEC 42001:2023 Lead Auditor", icon: <Diameter className="w-4 h-4 text-sky-600" /> },
    { title: "IBM Data Science Professional", icon: <Calculator className="w-4 h-4 text-sky-600" /> },
    { title: "IBM Data Analyst Professional", icon: <Database className="w-4 h-4 text-sky-600" /> },
    { title: "Google Cloud Digital Leader", icon: <Layout className="w-4 h-4 text-sky-600" /> },
    { title: "Deep Learning Specialization", icon: <Binary className="w-4 h-4 text-sky-600" /> },
    { title: "Generative AI with LLMs", icon: <Cpu className="w-4 h-4 text-sky-600" /> },
    { title: "UiPath_RPA Developer Diploma", icon: <Binoculars className="w-4 h-4 text-sky-600" /> }
  ];

  return (
    <div className="min-h-screen bg-slate-100/90 py-12 px-4 md:px-8 font-sans text-slate-800 antialiased selection:bg-sky-100 selection:text-slate-900">
      <div className="max-w-5xl mx-auto">
        
        {/* Header Section */}
        <header className="bg-white rounded-2xl shadow-sm p-6 md:p-10 mb-10 border border-slate-200/80 relative overflow-hidden">
          {/* 金融科技淡藍邊飾背景 */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-sky-50/50 rounded-full blur-3xl -z-10 pointer-events-none" />
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-10">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="mb-4">
                ...
                <p className="text-lg text-sky-700 font-bold mt-2 tracking-tight">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-6 text-sm text-slate-600 font-medium">
                <div className="flex items-center justify-center md:justify-start">
                  <Briefcase className="w-4 h-4 mr-2 text-sky-600" /> 11~12 Years of Experience
                </div>
                <div className="flex items-center justify-center md:justify-start">
                  <MapPin className="w-4 h-4 mr-2 text-sky-600" /> Daan Dist., Taipei City, Taiwan
                </div>
                <div className="flex items-center justify-center md:justify-start">
                  <Mail className="w-4 h-4 mr-2 text-sky-600" /> albertliu141@gmail.com
                </div>
                <div className="flex items-center justify-center md:justify-start">
                  <ShieldCheck className="w-4 h-4 mr-2 text-sky-600" /> CPA | PMP | IBM DS | AI Application Planner
                </div>
              </div>
              ...
              <div className="mt-7 flex flex-wrap justify-center md:justify-start gap-2">
                {["AI Governance", "MLOps", "Data Modeling", "Data Science", "Scrum"].map(skill => (
                  <span key={skill} className="bg-slate-900 text-sky-300 border border-slate-800 px-3.5 py-1 rounded-md text-xs font-mono font-medium tracking-wide shadow-sm">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <GraduationCap className="w-5 h-5 mr-2 text-sky-600" /> Education
              </h2>
              <div className="space-y-3">
                {education.map((edu, i) => (
                  <div key={i} className="p-4 rounded-xl border border-slate-200 bg-white shadow-sm hover:border-sky-300 transition-colors">
                    <p className="text-xs font-bold text-sky-700 mb-1">{edu.period}</p>
            ...
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <Award className="w-5 h-5 mr-2 text-sky-600" /> Certifications
              </h2>
              <div className="space-y-2.5">
                {certificates.map((cert, i) => (
                  <div key={i} className="flex items-center p-3 bg-white rounded-lg border border-slate-200 shadow-sm hover:border-slate-300 transition-colors">
                    {cert.icon}
                ...
                <div className="p-3 bg-sky-50/50 rounded-lg border border-sky-100">
                  <p className="text-[10px] text-sky-800 font-bold uppercase tracking-wider mb-1">Financial Licenses</p>
            ...
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <Code2 className="w-5 h-5 mr-2 text-sky-600" /> Technical Tools
              </h2>
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
                <div>
                  <p className="text-[11px] font-bold text-sky-700 uppercase mb-2 tracking-wider">Programming & DB</p>
                ...
                <div>
                  <p className="text-[11px] font-bold text-sky-700 uppercase mb-2 tracking-wider">Analysis & Visualization</p>
                  <div className="flex flex-wrap gap-1.5">
            ...
            {/* Patents Section (呈現科技感的類型標籤與統一的 2025 年份) */}
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <FileCheck className="w-5 h-5 mr-2 text-sky-600" /> Patents
              </h2>
              <div className="space-y-2.5">
                {patents.map((item, i) => (
                  <div key={i} className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm hover:border-sky-300 transition-colors">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold bg-sky-50 text-sky-700 px-2 py-0.5 rounded border border-sky-200/80">
                        {item.type}
                      </span>
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <Briefcase className="w-5 h-5 mr-2 text-sky-600" /> Work Experience
              </h2>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
                      <div className="flex flex-col md:flex-row justify-between items-start gap-3 mb-5">
                        <div>
                          ...
                          <p className="text-sky-700 font-bold text-base mt-0.5">{exp.role}</p>
                        ...
                        <div className="px-3 py-1 bg-slate-100 rounded text-slate-700 text-xs font-bold flex items-center whitespace-nowrap shrink-0 border border-slate-200">
                          <Calendar className="w-3.5 h-3.5 mr-1.5 shrink-0 text-sky-600" /> {exp.period}
                        </div>
                      </div>
                      ...
                      <ul className="space-y-2.5 mb-6">
                        {exp.details.map((detail, dIdx) => (
                          <li key={dIdx} className="flex text-xs md:text-sm text-slate-600 leading-relaxed">
                            <ChevronRight className="w-4 h-4 text-sky-600 mr-1.5 shrink-0 mt-0.5" />
                            <span>{detail}</span>
                          </li>
                        ))}
                      </ul>
                      ...
                      {exp.projects && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                          {exp.projects.map((proj, pIdx) => (
                            <div key={pIdx} className="bg-gradient-to-br from-slate-50 to-sky-50/40 p-3.5 rounded-lg border border-slate-200/80 hover:border-sky-200 transition-colors">
                              <p className="font-bold text-slate-900 text-xs flex items-center">
                                <TrendingUp className="w-3.5 h-3.5 mr-1.5 text-sky-600" /> {proj.name}
                              </p>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {exp.tags.map(tag => (
                          <span key={tag} className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-mono font-medium tracking-wider uppercase border border-slate-200/60">
                            #{tag}
                          </span>
                        ))}
                      </div>
