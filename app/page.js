...
const Resume = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100/80 py-12 px-4 md:px-8 font-sans text-slate-800 antialiased selection:bg-blue-100 selection:text-blue-900">
      <div className="max-w-5xl mx-auto">
        
        {/* Header Section */}
        <header className="bg-white rounded-2xl shadow-sm p-6 md:p-10 mb-10 border border-slate-200/80">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-10">
            {/* Profile Image Container */}
            <div className="relative shrink-0">
              <div className="w-36 h-36 md:w-44 md:h-44 rounded-xl overflow-hidden border-2 border-slate-100 shadow-md">
                <img 
                  src="/profile.jpg" 
                  alt="Albert Profile" 
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="mb-4">
                ...
                <p className="text-lg text-blue-700 font-bold mt-2 tracking-tight">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-6 text-sm text-slate-600 font-medium">
                <div className="flex items-center justify-center md:justify-start">
                  <Briefcase className="w-4 h-4 mr-2 text-blue-600" /> 11~12 Years of Experience
                </div>
                <div className="flex items-center justify-center md:justify-start">
                  <MapPin className="w-4 h-4 mr-2 text-blue-600" /> Daan Dist., Taipei City, Taiwan
                </div>
                <div className="flex items-center justify-center md:justify-start">
                  <Mail className="w-4 h-4 mr-2 text-blue-600" /> albertliu141@gmail.com
                </div>
                <div className="flex items-center justify-center md:justify-start">
                  <ShieldCheck className="w-4 h-4 mr-2 text-blue-600" /> CPA | PMP | IBM DS | AI Application Planner
                </div>
              </div>
              ...
              <div className="mt-7 flex flex-wrap justify-center md:justify-start gap-2">
                {["AI Governance", "MLOps", "Data Modeling", "Data Science", "Scrum"].map(skill => (
                  <span key={skill} className="bg-slate-900 text-slate-100 px-3.5 py-1 rounded-md text-xs font-semibold tracking-wide shadow-sm">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <section>
              ...
              <div className="space-y-3">
                {education.map((edu, i) => (
                  <div key={i} className="p-4 rounded-xl border border-slate-200/90 bg-white shadow-sm hover:border-blue-300 transition-colors">
            ...
            <section>
              ...
              <div className="space-y-2.5">
                {certificates.map((cert, i) => (
                  <div key={i} className="flex items-center p-3 bg-white rounded-lg border border-slate-200/90 shadow-sm">
                    {cert.icon}
            ...
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <Code2 className="w-5 h-5 mr-2 text-blue-600" /> Technical Tools
              </h2>
              <div className="bg-white p-5 rounded-xl border border-slate-200/90 shadow-sm space-y-4">
                <div>
                  <p className="text-[11px] font-bold text-blue-700 uppercase mb-2 tracking-wider">Programming & DB</p>
                ...
                <div>
                  <p className="text-[11px] font-bold text-blue-700 uppercase mb-2 tracking-wider">Analysis & Visualization</p>
                  <div className="flex flex-wrap gap-1.5">
            ...
            {/* Patents Section (呈現動態類型標籤) */}
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center tracking-tight border-b border-slate-200 pb-2">
                <FileCheck className="w-5 h-5 mr-2 text-blue-600" /> Patents
              </h2>
              <div className="space-y-2.5">
                {patents.map((item, i) => (
                  <div key={i} className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-sm hover:border-blue-300 transition-colors">
              ...
              <div className="space-y-8">
                {experiences.map((exp, idx) => (
                  <div key={idx} className="relative">
                    <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200/90 shadow-sm">
                      <div className="flex flex-col md:flex-row justify-between items-start gap-3 mb-5">
                        <div>
                          ...
                          <p className="text-blue-700 font-bold text-base mt-0.5">{exp.role}</p>
                          <p className="text-xs text-slate-500 font-medium mt-0.5">{exp.size}</p>
                        </div>
                        
                        <div className="px-3 py-1 bg-slate-100 rounded text-slate-700 text-xs font-bold flex items-center whitespace-nowrap shrink-0 border border-slate-200/80">
                      ...
                      <ul className="space-y-2.5 mb-6">
                        {exp.details.map((detail, dIdx) => (
                          <li key={dIdx} className="flex text-xs md:text-sm text-slate-600 leading-relaxed">
                            <ChevronRight className="w-4 h-4 text-blue-600 mr-1.5 shrink-0 mt-0.5" />
                            <span>{detail}</span>
                          </li>
                        ))}
                      </ul>
                      ...
                      {exp.projects && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                          {exp.projects.map((proj, pIdx) => (
                            <div key={pIdx} className="bg-blue-50/40 p-3.5 rounded-lg border border-blue-100/80 hover:bg-blue-50/70 transition-colors">
                              ...
                              <p className="text-xs text-blue-800 mt-1.5 leading-relaxed">{proj.desc}</p>
                            </div>
                          ))}
                        </div>
                      )}
